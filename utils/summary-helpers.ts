export const parseSection = (section: string): { title: string; points: string[] } => {
  ...
  // 🔥 HERE: Split long points into multiple emoji lines
  const expandedPoints = points.flatMap((point) =>
    point
      .split(/\.\s+(?=[^\s])/g) // naive fallback to avoid Unicode escape issues
      .map((p) => p.trim())
      .filter(Boolean)
  );
...
export function parseEmojiPoint(content: string) {
  ...
  const matches = cleanContent.match(/^(.{1,3})\s+(.+)$/u); // Emoji fallback
